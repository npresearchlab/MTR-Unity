...
public abstract class AbstractSerialThread
{
    ...
    // Indicates whether the arduino is connected or not.
    ...
    {
        outputQueue.Enqueue((byte)message);
    }
    ...
    public void RunForever()
    {
        ...
        try
        {
            ...
            {
                SendToWire(new byte[] { (byte)outputQueue.Dequeue() }, serialPort);
            }
    ...
    private void RunOnce()
    {
        try
        {
            ...
            if (inputMessage != null)
            {
                ...
                {
                    Debug.LogWarning("Queue is full (Max of " + maxUnreadMessages + " messages). Dropping message: " + inputMessage);
                }
            }
        }
    ...
    // ------------------------------------------------------------------------
    protected abstract void SendToWire(byte[] message, SerialPort serialPort);
